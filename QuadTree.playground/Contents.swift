...
func zoomLevel(fromScale scale: MKZoomScale) -> Int {
	let tileCount = MKMapSize.world.width / 256.0
...
func clusteredAnnotations(in rect: MKMapRect,
	...
	let minX: Double = floor(rect.minX * scaleFactor)
	let maxX: Double = floor(rect.maxX * scaleFactor);
	let minY = floor(rect.minY * scaleFactor);
	let maxY = floor(rect.maxY * scaleFactor);
	
	var annotations: [MKAnnotation] = []
	
	for x in stride(from: minX, through: maxX, by: 1.0) {
		for y in stride(from: minY, through: maxY, by: 1.0) {
			let mapRect: MKMapRect = MKMapRect(x: x/scaleFactor,
											   y: y/scaleFactor,
